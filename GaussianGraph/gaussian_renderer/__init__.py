...
# from submodules.ashawkey_diff_gaussian_rasterization.ashawkey_diff_gaussian_rasterization.__init__ import GaussianRasterizationSettings, GaussianRasterizer 
...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, iteration,
    ...
    # [Safety] Sanitize geometry to prevent crashes
    if torch.isnan(means3D).any() or torch.isinf(means3D).any():
        means3D = torch.nan_to_num(means3D, nan=0.0)
    if torch.isnan(opacity).any() or torch.isinf(opacity).any():
        opacity = torch.nan_to_num(opacity, nan=0.1)
    ...
    # [Safety] Sanitize scales and rotations
    if scales is not None:
        if torch.isnan(scales).any() or torch.isinf(scales).any():
            scales = torch.nan_to_num(scales, nan=0.01)
        scales = torch.clamp(scales, max=100.0)
    
    if rotations is not None:
        if torch.isnan(rotations).any() or torch.isinf(rotations).any():
            rotations = torch.nan_to_num(rotations, nan=0.0)
            rotations[:, 0] = 1.0
    ...
    if render_feat_map:
        ...
        # [Safety] Sanitize ins_feat
        if torch.isnan(ins_feat).any() or torch.isinf(ins_feat).any():
            ins_feat = torch.nan_to_num(ins_feat, nan=0.0)

        # [Safety] Near plane and Frustum culling to prevent rasterizer crash
        with torch.no_grad():
            view_matrix = viewpoint_camera.world_view_transform
            means3D_hom = torch.cat((means3D, torch.ones_like(means3D[:, :1])), dim=1)
            means3D_view = means3D_hom @ view_matrix
            valid_depth_mask = means3D_view[:, 2] > 0.2
            
            # Frustum culling
            p_hom = means3D_hom @ viewpoint_camera.full_proj_transform
            p_w = 1.0 / (p_hom[:, 3] + 0.0000001)
            p_proj = p_hom[:, :3] * p_w.unsqueeze(1)
            valid_frustum_mask = (p_proj[:, 0] > -1.3) & (p_proj[:, 0] < 1.3) & (p_proj[:, 1] > -1.3) & (p_proj[:, 1] < 1.3)
            
            valid_mask = valid_depth_mask & valid_frustum_mask
        
        if valid_mask.sum() < means3D.shape[0]:
            means3D = means3D[valid_mask].contiguous()
            means2D = means2D[valid_mask].contiguous()
            opacity = opacity[valid_mask].contiguous()
            scales = scales[valid_mask].contiguous()
            rotations = rotations[valid_mask].contiguous()
            if cov3D_precomp is not None:
                cov3D_precomp = cov3D_precomp[valid_mask].contiguous()
            ins_feat = ins_feat[valid_mask].contiguous()
        ...
        rendered_ins_feat, _, _, _ = rasterizer(
            ...
            means2D = means2D,
            shs = None,
            colors_precomp = ins_feat[:, :3].contiguous(),   # render features as pre-computed colors
            opacities = opacity,
        ...
        if ins_feat.shape[-1] > 3:
            rendered_ins_feat2, _, _, _ = rasterizer(
                ...
                means2D = means2D,
                shs = None,
                colors_precomp = ins_feat[:, 3:6].contiguous(),  # render features as pre-computed colors
                opacities = opacity,
                scales = scales * rescale_factor,
            ...
            # [Fix] Ensure tensors are on the same device before concatenation
            if rendered_ins_feat.device != rendered_ins_feat2.device:
                rendered_ins_feat2 = rendered_ins_feat2.to(rendered_ins_feat.device)
    ...
    # Check if viewed_pts is valid
    has_viewed_pts = viewed_pts is not None and viewed_pts.sum() != 0

    if render_cluster and cluster_idx is not None and has_viewed_pts:
        ins_feat = (pc.get_ins_feat(origin=origin_feat) + 1) / 2   # pseudo -> norm, else -> raw
        rendered_clusters = []
